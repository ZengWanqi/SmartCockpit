...
import face_seat_control  # 导入座椅控制模块
...
def main():
    ...
    feature_extractor = face_feature_extract.FaceFeatureExtractor(common_context)  # 创建特征提取对象
    face_seat_controlr = face_seat_control.FaceSeatControl(common_context)  # 创建座椅控制对象
    face_recognizer = face_recognition.FaceRecognition(common_context, face_seat_controlr, feature_extractor)  # 创建人脸识别对象
    face_recognizer.run()  # 调用运行方法
